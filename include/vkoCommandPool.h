...
public:
  ...
  void configure(uint32 in_queueFamily, VkCommandPoolCreateFlags in_flags= 0);
  ...
  void addCommandBuffers(uint32 in_nr, VkCommandBufferLevel in_level, VkCommandBuffer *out_buffers);

  void freeCommandBuffer(VkCommandBuffer in_buffer);                    // freeing is not required if the pool is destroyed
  void freeCommandBuffers(uint32 in_nr, VkCommandBuffer *in_buffer);    // freeing is not required if the pool is destroyed
