...
public:
  ...
  inline void cfgDimensions(uint32_t in_dx, uint32_t in_dy, int32_t in_layers= 1) { dx= in_dx, dy= in_dy, layers= in_layers; }

  // must set the renderpass that works with this framebuffer
  inline void cfgRenderPass(const VkRenderPass in_r) { renderPass= in_r; }
  ...
  void cfgAddAttachment(VkImage in_image, VkFormat in_imageFormat, VkImageAspectFlags in_aspect, uint32_t in_baseLayer= 0, uint32_t in_nrLayers= 1);
