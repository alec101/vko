...
public:
  ...
  VkoSet *addDescriptorSet(uint32_t in_setNumber);       // can be used to manually add an empty VkoSet
  ...
  VkoDescriptor *addDescriptor(uint32_t in_set, uint32_t in_binding, VkDescriptorType in_type, uint32_t in_count, VkShaderStageFlags in_stages, VkSampler *in_pImutableSampler= nullptr);
  ...
  VkoSet *getSet(uint32_t index);       // <index>: the set index, counted from the first set created with this object.
  ...
  bool buildSet(uint32_t in_setIndex);  // 
  ...
  void freeSet(uint32_t index);         // frees and deletes all data about a specific set. Specs say it's dangerous to use if adding new sets that are not the same/compatible like the one freed. Use resetPool() and re-create the descriptors is safer
